...
/// titles missing from vector_state, and this task never runs the LLM steps --
/// those cost money, so they stay a deliberate action on the settings page.
/// </summary>
public class RefreshCorpusTask(ILogger<RefreshCorpusTask> logger) : IScheduledTask
{
    ...
    public string Description =>
        "Reads new titles from your library and builds their search vectors. " +
        "Free, and safe to run as often as you like: each step only does outstanding " +
        "work, so nothing already indexed is redone. This never calls an LLM — " +
        "enriching new titles costs money and stays a manual action on the Bloom " +
        "settings page, which shows how many are waiting and what they would cost.";
