    ...
    ILogger<JellyfinSearchAdapter> logger)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (server != null)
        {
            ...
            if (serverTypes.Count > 0)
            {
                ...
                if (serverIds != null)
                {
                    var (page, serverTotal) = Paginate(serverIds, offset, limit);
                    ...
                    if (!ShouldFallThrough(page.Count, offset))
    ...
    /// <summary>
    /// Slice the server's ranked ids into the requested page, and estimate a
    /// total. The server does not report a total, so a short result means we
    /// have seen everything and anything else is "at least one more page" --
    /// under-reporting would stop a client paging before the results run out.
    /// </summary>
    internal static (List<Guid> Page, int Total) Paginate(
        IReadOnlyList<Guid> ids, int offset, int limit)
    {
        var page = ids.Skip(offset).Take(limit).ToList();
        var exhausted = ids.Count < offset + limit;
        return (page, exhausted ? ids.Count : offset + limit + limit);
    }

    /// <summary>
    /// Whether an empty page should hand over to keyword search.
    ///
    /// Nothing on the first page means the server had no answer, so keyword
    /// deserves a turn. Nothing on a later page is the genuine end of results,
    /// and falling through there would resurface junk the server already
    /// rejected -- as a second page of unrelated titles.
    /// </summary>
    internal static bool ShouldFallThrough(int pageCount, int offset)
        => pageCount == 0 && offset == 0;
