...
{
    /// <summary>
    /// Template v4: {genres}. {tags}. {people}. {overview} {tagline} {name} ({originalTitle}), {type} {year}.
    ...
    /// The original-language title rides alongside the localized one so a film
    /// is reachable under the name it was released with — the keyword layer has
    /// always indexed it, the vector layer could not see it.
    ...
    public static string BuildEmbedText(MeilisearchItem item)
    {
        ...
        if (!string.IsNullOrWhiteSpace(item.OriginalTitle)
            && !string.Equals(item.OriginalTitle, item.Name, StringComparison.OrdinalIgnoreCase))
            sb.Append(" (").Append(item.OriginalTitle).Append(')');
        sb.Append(", ").Append(FriendlyType(item.Type));
