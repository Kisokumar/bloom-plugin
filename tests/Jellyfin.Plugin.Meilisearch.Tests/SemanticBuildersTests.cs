...
public class SemanticBuildersTests
{
    ...
    [Fact]
    public void Embed_text_carries_a_differing_original_title()
    {
        var item = Item("Spirited Away") with { OriginalTitle = "Sen to Chihiro no Kamikakushi" };
        Assert.Contains("Spirited Away (Sen to Chihiro no Kamikakushi), Movie 2007.",
            IndexDocumentBuilder.BuildEmbedText(item));
    }

    [Fact]
    public void Embed_text_omits_an_original_title_that_repeats_the_name()
    {
        var item = Item() with { OriginalTitle = "superbad" };
        Assert.Contains("Superbad, Movie 2007.", IndexDocumentBuilder.BuildEmbedText(item));
    }
